...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('Purchase') and o.name = 'FK_PURCHASE_PRINTING_RELEASE')
alter table Purchase
   drop constraint FK_PURCHASE_PRINTING_RELEASE
go

if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('Purchase') and o.name = 'FK_PURCHASE_PURCHASE_USER')
alter table Purchase
   drop constraint FK_PURCHASE_PURCHASE_USER
go
...
if exists (select 1
           ...
           where  id    = object_id('Purchase')
            ...
            and   indid > 0
            and   indid < 255)
   drop index Purchase.Printing_FK
go
...
create table Purchase (
   ...
   movieId              int                  not null,
   userId               int                  not null,
   constraint PK_PURCHASE primary key (cinemaId, movieId, userId)
)
go
...
/* Index: Printing_FK                                           */
/*==============================================================*/
create index Printing_FK on Purchase (
cinemaId ASC,
movieId ASC
)
go
...
alter table Purchase
   add constraint FK_PURCHASE_PRINTING_RELEASE foreign key (cinemaId, movieId)
      references Release (cinemaId, movieId)
go

alter table Purchase
   add constraint FK_PURCHASE_PURCHASE_USER foreign key (userId)
      references "User" (userId)
go
